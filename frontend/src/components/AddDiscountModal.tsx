...
export default function AddDiscountModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        onAdd();
        onFeedback("Diskon berhasil ditambahkan!");
        onClose();
      } else {
        setError(data.message || "Gagal tambah diskon");
        onFeedback("Gagal tambah diskon.");
      }
    } catch {
      setError("Gagal koneksi ke server");
      onFeedback("Gagal tambah diskon (server error).");
    } finally {
      setLoading(false);
    }
  };
